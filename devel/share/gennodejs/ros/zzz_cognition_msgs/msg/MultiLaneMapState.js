...
class MultiLaneMapState {
  ...
  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type MultiLaneMapState
    // Serialize message field [distance_to_junction]
    bufferOffset = _serializer.float32(obj.distance_to_junction, buffer, bufferOffset);
    ...
    obj.lanes.forEach((val) => {
      bufferOffset = LaneState.serialize(val, buffer, bufferOffset);
    });
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type MultiLaneMapState
    let len;
    let data = new MultiLaneMapState(null);
    // Deserialize message field [distance_to_junction]
    data.distance_to_junction = _deserializer.float32(buffer, bufferOffset);
    // Deserialize message field [lanes]
    // Deserialize array length for message field [lanes]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.lanes = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.lanes[i] = LaneState.deserialize(buffer, bufferOffset)
    }
  ...
  static getMessageSize(object) {
    ...
    object.lanes.forEach((val) => {
      length += LaneState.getMessageSize(val);
    });
  ...
  static Resolve(msg) {
    ...
    const resolved = new MultiLaneMapState(null);
    ...
    if (msg.lanes !== undefined) {
      ...
      for (let i = 0; i < resolved.lanes.length; ++i) {
        resolved.lanes[i] = LaneState.Resolve(msg.lanes[i]);
      }
    }
    else {
      resolved.lanes = []
...
module.exports = MultiLaneMapState;
