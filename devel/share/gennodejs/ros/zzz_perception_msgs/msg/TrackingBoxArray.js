...
class TrackingBoxArray {
  ...
  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type TrackingBoxArray
    ...
    obj.targets.forEach((val) => {
      bufferOffset = TrackingBox.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type TrackingBoxArray
    let len;
    let data = new TrackingBoxArray(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.targets[i] = TrackingBox.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.targets.forEach((val) => {
      length += TrackingBox.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new TrackingBoxArray(null);
    ...
    if (msg.targets !== undefined) {
      ...
      for (let i = 0; i < resolved.targets.length; ++i) {
        resolved.targets[i] = TrackingBox.Resolve(msg.targets[i]);
      }
    }
    else {
      resolved.targets = []
...
module.exports = TrackingBoxArray;
