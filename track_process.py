...
meiyuan: 442866.361698,4427904.89691
...
def process_bags(args):
    ...
    for topic, msg, _ in input_bag.read_messages(topics=['/zzz/navigation/ego_pose', 
        ...
        if topic == '/zzz/navigation/ego_pose':
            point = (msg.state.pose.pose.position.x, msg.state.pose.pose.position.y)
            if last_point is None:
                last_point = point
    ...
    np.savetxt(args.output, output_data, fmt="%.8f,%.8f", delimiter=',', newline='\n')
