...
#define MAX_PATH 260
  ...
  void Pre_Lat_Controller::controlInit()
  {
    ...
    _kappa_path_angle_gain = getParam<double>(Controlparam, "_kappa_path_angle_gain", 0);
    
    _dis_pre_max = getParam<double>(Controlparam, "_dis_pre_max", 0);
    _dis_pre_min = getParam<double>(Controlparam, "_dis_pre_min", 0);
    
    _d_dis_min_k = getParam<double>(Controlparam, "_d_dis_min_k", 0);
    _d_dis_max_k = getParam<double>(Controlparam, "_d_dis_max_k", 0);
    ...
    feedforward_K_turn = getParam<double>(Controlparam, "feedforward_K_turn", 0);
    feedforward_K_line = getParam<double>(Controlparam, "feedforward_K_line", 0);
    //meanfilter
                                        ...
                                        double _vehicle_v)
  {
    ...
    if (_path_x.size() > 2 && _path_y.size() > 2 & _path_theta.size() > 2 && _path_curvature.size() > 2)
    {
      ...
      {
        // fix927
        feedforward_K = feedforward_K_turn;
        _d_dis = _t_preview * _vehicle_v + _d_0;
        
      }
      else
      {
        feedforward_K = feedforward_K_line;
      ...
      if (nextpoint_num_1 < _path_curvature.size())
      {
        curvature_angle = _kappa_path_angle_gain * curvature_gain * atan(L * kappa_trajectory) * K / M_PI * 180;
...
//
      ...
      //
      // ofstream out;
      // out.open("/home/fangniuwa/g3bug_fix/control_fix/src/Control/follow.txt", std::ios::out | std::ios::app);
