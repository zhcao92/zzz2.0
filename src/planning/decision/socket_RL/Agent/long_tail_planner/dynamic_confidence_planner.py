...
class DCP_Agent():
    ...
    def act(self, candidate_trajectories_tuple, dynamic_map):
        ...
        return dcp_action, worst_Q_list, worst_Q_list,fast_action
    ...
    def __init__(self, ensemble_num, history_frame, future_frame, agent_dimension, agent_num, obs_bias_x, obs_bias_y, 
        ...
        for i in range(self.ensemble_num):
            env_transition = TrajPredGaussion(self.history_frame * self.agent_dimension * self.agent_num,
                                              self.future_frame * 2 * self.agent_num, hidden_unit=256)
        ...
        if len(self.data) > 0:
            ...
            for k in range(1):
                ...
                for i in range(self.ensemble_num):
                    ...
                    print("------------loss", loss,i)
                    with open("Agent/long_tail_planner/save_models/loss_record.txt", 'a') as fw:
                        fw.write(str(loss)) 
                        fw.write(", ")
                        fw.write(str(i)) 
                        fw.write("\n")
    ...
    def save(self, train_step):
        for i in range(self.ensemble_num):
            torch.save(
                ...
                'Agent/long_tail_planner/save_models/ensemble_models_%s_%s.pt' % (train_step, i)
            )
