...
class DynamicMap():
    ...
    def update_map_from_zzz_MapState(self,MapState):
        self.init_dynamic_map()
        _M = MapState
        # Ego information from obs
        self.ego_vehicle.x = _M.ego_state.pose.pose.position.x
        self.ego_vehicle.y = _M.ego_state.pose.pose.position.y
        self.ego_vehicle.vx = _M.ego_state.twist.twist.linear.x
        self.ego_vehicle.vy = _M.ego_state.twist.twist.linear.y
        self.ego_vehicle.lane_idx = 0  # not know
        self.ego_vehicle.v = math.sqrt(self.ego_vehicle.vx ** 2 + self.ego_vehicle.vy ** 2)
        # Check here !!!! the yaw 
        self.ego_vehicle.yaw = self.cal_yaw(self.ego_vehicle.vy,self.ego_vehicle.vx)

        for obs in _M.jmap.obstacles: 
            # distance
            vehicle = Vehicle()
            vehicle.x = obs.state.pose.pose.position.x
            vehicle.y = obs.state.pose.pose.position.y
            vehicle.vx = obs.state.twist.twist.linear.x
            vehicle.vy = obs.state.twist.twist.linear.y
            vehicle.yaw = self.cal_yaw(vehicle.vy,vehicle.vx)
            vehicle.lane_idx = 0
            vehicle.v = math.sqrt(vehicle.vx ** 2 + vehicle.vy ** 2)
            self.vehicles.append(vehicle)

    def cal_yaw(self,vx,vy): # TODO: check !!!
        # check
