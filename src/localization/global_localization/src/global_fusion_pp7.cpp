...
* @copyright None 
* None
* @last-modified 2020-05-19T18:36:45.665Z+08:00  car Coordinate system: x_forword  y_left  z_up
*/
...
#define gyro_scale_factor ((0.008 / 65536) / 125 * 125) * (M_PI / 180)
#define acc_scale_factor ((0.2 / 65536) / 125 * 125) * 9.8065 * 0.001

namespace localization
{
    class global_fusion_pp7
    {
    private:
        ros::NodeHandle n;
        //sub
    ...
    public:
        global_fusion_pp7()
        {
            ...
            //in
            n.getParam("gnss_inspvax_topic", gnss_inspvax_topic);
            ...
            n.getParam("gnss_heading2b_topic", gnss_heading2b_topic);
    ...
    **/
        ...
        {
            //subscribe
            sub_fix = n.subscribe(gnss_inspvax_topic, 100, &global_fusion_pp7::inspvax_callback, this);
            sub_imu = n.subscribe(gnss_rawimu_topic, 100, &global_fusion_pp7::rawimu_callback, this);
            sub_vel = n.subscribe(gnss_bestposb_topic, 100, &global_fusion_pp7::bestposb_callback, this);
            sub_odom = n.subscribe(gnss_bestvelb_topic, 100, &global_fusion_pp7::bestvelb_callback, this);
            sub_pos_type = n.subscribe(gnss_heading2b_topic, 100, &global_fusion_pp7::heading2b_callback, this);
        ...
        void att2q(const double pitch, const double roll, const double yaw,
        ...
        void inspvax_callback(const novatel_gnss_msgs::InsPvax &InsPvax_msg)
        {
            double W = 0, X = 0, Y = 0, Z = 0;
            double roll, pitch, yaw;
            roll = InsPvax_msg.roll * M_PI / 180;
            pitch = InsPvax_msg.pitch * M_PI / 180;
            yaw = InsPvax_msg.azimuth * M_PI / 180;
            yaw = yaw +   M_PI / 180;
            ...
            att2q(pitch, roll, yaw, &W, &X, &Y, &Z);

            //gps/fix
            new_fix.header = InsPvax_msg.header;
            new_fix.header.frame_id = "wgs84";
            new_fix.longitude = InsPvax_msg.longitude;
            new_fix.latitude = InsPvax_msg.latitude;
            new_fix.altitude = InsPvax_msg.height;
            new_fix.position_covariance[0] = InsPvax_msg.longitude_std;
            new_fix.position_covariance[4] = InsPvax_msg.latitude_std;
            new_fix.position_covariance[8] = InsPvax_msg.height_std;
            if(InsPvax_msg.pos_type==56) 
                new_fix.status.status = 2;
            else 
                new_fix.status.status = 0;
            pub_fix.publish(new_fix); //fix

            ///For vel
            new_vel.header = InsPvax_msg.header;
            new_vel.header.frame_id = "enu";
            new_vel.twist.twist.linear.x = InsPvax_msg.east_velocity;
            new_vel.twist.twist.linear.y = InsPvax_msg.north_velocity;
            new_vel.twist.twist.linear.z = InsPvax_msg.up_velocity;
            new_vel.twist.covariance[0] = InsPvax_msg.east_velocity_std;
            new_vel.twist.covariance[4] = InsPvax_msg.north_velocity_std;
            new_vel.twist.covariance[8] = InsPvax_msg.up_velocity_std;
            ...
            //old version
            // new_imu.linear_acceleration.x = RawImu.y_velocity_change_neg*acc_scale_factor*100;
            ...
            // new_imu.angular_velocity.x = RawImu.y_angle_change_neg*gyro_scale_factor*100;
            ...
            new_imu.linear_acceleration.x = RawImu.y_velocity_change_neg * acc_scale_factor;
            new_imu.linear_acceleration.y = RawImu.x_velocity_change * acc_scale_factor * (-1);
            new_imu.linear_acceleration.z = RawImu.z_velocity_change * acc_scale_factor;
            new_imu.angular_velocity.x = RawImu.y_angle_change_neg * gyro_scale_factor;
            new_imu.angular_velocity.y = RawImu.x_angle_change * gyro_scale_factor * (-1);
            new_imu.angular_velocity.z = RawImu.z_angle_change * gyro_scale_factor;
            ...
            new_twist.twist.linear.x = new_vel.twist.twist.linear.x;
            new_twist.twist.linear.y = new_vel.twist.twist.linear.y;
            new_twist.twist.linear.z = new_vel.twist.twist.linear.z;

            new_twist.twist.angular.x = new_imu.angular_velocity.x;
            new_twist.twist.angular.y = new_imu.angular_velocity.y;
            new_twist.twist.angular.z = new_imu.angular_velocity.z;
            RAWimu_mutex.unlock();
            ...
            double utm_y;
            std::string utm_zone;
            gps_common::LLtoUTM(InsPvax_msg.latitude, InsPvax_msg.longitude, utm_y, utm_x, utm_zone);

            new_odom.header = InsPvax_msg.header;
            ...
            new_odom.pose.pose.position.x = utm_x - utm0_x;
            new_odom.pose.pose.position.y = utm_y - utm0_y;
            new_odom.pose.pose.position.z = 0; //33.0999259949;
            pub_odom.publish(new_odom);

            //for ego_pose
            zzz_driver_msgs::RigidBodyStateStamped state;
            state.header.frame_id = "map";
            state.header.stamp = InsPvax_msg.header.stamp;
            state.state.child_frame_id = "odom";
            state.state.pose.pose = new_odom.pose.pose;
            state.state.twist.twist.linear = new_twist.twist.linear;

            state.state.twist.twist.angular = new_twist.twist.angular;
            state.state.accel.accel.linear = new_imu.linear_acceleration;
        ...
        void heading2b_callback(const novatel_gnss_msgs::Heading2b &Heading2b_msg)
        {
        }
...
{
    ros::init(argc, argv, "global fusion pp7");
    ROS_INFO("\033[1;32m---->\033[0m global_fusion pp7 node Started.");

    localization::global_fusion_pp7 GF;
    ros::spin();
    return 0;
}