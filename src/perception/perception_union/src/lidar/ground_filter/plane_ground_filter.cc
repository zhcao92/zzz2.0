...
bool PlaneGroundFilter::init(std::string& config) {
  ...
  std::string config_path = config + "/config/ground_filter.yaml";
...
void PlaneGroundFilter::filter(const pcl::PointCloud<pcl::PointXYZI>::Ptr& in_cloud_ptr, 
  ...
  for(int i = 0; i < column ; i++ ) {
    for(int j = 0; j < row ; j++) {
      ...
      double sum = 0;
      int cnt = 0;
      for (int k = 0; k < grid_num && cnt < num_lpr_ ; k++) {     // num_lpr_ = 8
      ...
      if(grid_num < 0.0005 * points_num) {
        ...
        for (int k = 0; k < grid[i][j].grid_cloud->points.size(); k++) {
          if (grid[i][j].grid_cloud->points[k].z < lpr_height + th_seeds_) {     //th_seeds = 0.50
        ...
        for (int l = 0; l < num_iter_; l++) {
          ...
          double d_ = -(normal_.transpose() * seeds_mean)(0, 0);
...
//            aved /= seed_result.rows();
            ...
            for (int r = 0; r < seed_result.rows(); r++) {
              adatp_threVal += (seed_result[r] + d_) * (seed_result[r] + d_);
            }
            ...
            for (int r = 0; r < result.rows(); r++) {
              if (result[r] < 1.5 * adatp_threVal - d_ + 0.1) {
