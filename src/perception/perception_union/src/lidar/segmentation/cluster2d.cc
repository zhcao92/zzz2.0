...
#define _OBJECT_ANGLE_BOX_
#ifndef _OBJECT_ANGLE_BOX_
#include "common/bounding_line.h"
#endif // _OBJECT_ANGLE_BOX_
...
// length, width, height
float ObjectDimThreshold[5][3] {
1.0f, 1.0f, 1.0f, // unknown
4.0f, 2.0f, 2.0f, // big monitor
3.5f, 1.8f, 1.6f, // small monitor
1.8f, 1.0f, 1.6f, // Cyclist
1.0f, 1.0f, 1.2f  // Pedestrian
} ;

bool Cluster2D::init(int rows, int cols, float range) {
  for (auto i=0; i<5; i++) {
    for (auto j=0; j<5; j++) {
      object_dim_threshold_[i][j] = ObjectDimThreshold[i][j];
    }
  }
...
// #define _OBJECT_ANGLE_BOX_
#ifdef _OBJECT_ANGLE_BOX_
perception::Object Cluster2D::obstacleToObject(const Obstacle& in_obstacle, 
    const perception::Header& in_header) {
  perception::Object res_obj;
  ...
  res_obj.valid = true; 
  res_obj.header = in_header;
  res_obj.score = in_obstacle.score;
  res_obj.label = static_cast<int>(in_obstacle.meta_type);
  res_obj.point_num = static_cast<int>(in_cluster.points.size());
  float length_threshold = object_dim_threshold_[res_obj.label][0];
  float width_threshold = object_dim_threshold_[res_obj.label][1];
  float height_threshold = object_dim_threshold_[res_obj.label][2]; 
  res_obj.yaw = in_obstacle.yaw;
  res_obj.direction.x = std::cos(res_obj.yaw);
  res_obj.direction.y = std::sin(res_obj.yaw);
  res_obj.direction.z = 0.0f;
  Eigen::Matrix2f project_matrix;
  project_matrix << res_obj.direction.x, res_obj.direction.y, 
      -res_obj.direction.y, res_obj.direction.x;
  for (auto pit = in_cluster.points.begin(); 
      pit != in_cluster.points.end(); ++pit) {
    float px = pit->x;
    float py = pit->y;
    float pz = pit->z;
    sum_x += px;
    sum_y += py;
    sum_z += pz;
    Eigen::Vector2f pt_raw, pt;
    pt_raw << px, py;
    pt = project_matrix * pt_raw;
    // pt = pt_raw;
  ...
  if (length < length_threshold) {
    ...
    if (abs(max_x) > abs(min_x)) {
      max_x = min_x + length_threshold;
    } else {
      min_x = max_x - length_threshold;
    }
  }
  if (width < width_threshold) {
    width = width_threshold;
    if (abs(max_y) > abs(min_y)) {
      max_y = min_y + width_threshold;
    } else {
      min_y = max_y - width_threshold;
    }
  }
  if (height < height_threshold) {
      height = height_threshold;
      min_z = max_z - height_threshold;
  }
  Eigen::Vector2f pose2d, pose2d_raw;
  Eigen::Matrix2f project_matrix_inv;
  project_matrix_inv << res_obj.direction.x, -res_obj.direction.y, 
      res_obj.direction.y, res_obj.direction.x;
  float num_f = static_cast<float>(in_cluster.points.size());
  pose2d_raw[0] = (max_x + min_x) * 0.5;
  pose2d_raw[1] = (max_y + min_y) * 0.5;
  pose2d = project_matrix_inv * pose2d_raw;  
  res_obj.barycenter.x = sum_x / num_f;
  res_obj.barycenter.y = sum_y / num_f;
  res_obj.barycenter.z = sum_z / num_f;  
  res_obj.center.x = pose2d[0];
  res_obj.center.y = pose2d[1];
  res_obj.center.z = (max_z + min_z) * 0.5f;
  // res_obj.pose = res_obj.center;
  res_obj.pose.x = static_cast<double>(res_obj.center.x);
  res_obj.pose.y = static_cast<double>(res_obj.center.y);
  res_obj.pose.z = static_cast<double>(res_obj.center.z);
  pose2d_raw << max_x, max_y;
  pose2d = project_matrix_inv * pose2d_raw;
  res_obj.corners[0] = {pose2d.x(), pose2d.y()};
  pose2d_raw << min_x, max_y;
  pose2d = project_matrix_inv * pose2d_raw;
  res_obj.corners[1] = {pose2d.x(), pose2d.y()};
  pose2d_raw << min_x, min_y;
  pose2d = project_matrix_inv * pose2d_raw;
  res_obj.corners[2] = {pose2d.x(), pose2d.y()};
  pose2d_raw << max_x, min_y;
  pose2d = project_matrix_inv * pose2d_raw;
  res_obj.corners[3] = {pose2d.x(), pose2d.y()};
  res_obj.dims.x = static_cast<double>(length);
  res_obj.dims.y = static_cast<double>(width);
  res_obj.dims.z = static_cast<double>(height);
  Eigen::AngleAxisd yawAngle(Eigen::AngleAxisd(static_cast<double>(res_obj.yaw), 
      Eigen::Vector3d::UnitZ()));
  Eigen::Quaterniond quaternion;
  quaternion = yawAngle;
  res_obj.orie.x = quaternion.x();
  res_obj.orie.y = quaternion.y();
  res_obj.orie.z = quaternion.z();
  res_obj.orie.w = quaternion.w();
  return res_obj;
}
#else // _OBJECT_ANGLE_BOX_
perception::Object
Cluster2D::obstacleToObject(const Obstacle& in_obstacle, const perception::Header& in_header)
{
    perception::Object res_obj;
    pcl::PointCloud<pcl::PointXYZI> in_cluster = *in_obstacle.cloud_ptr;
    res_obj.header = in_header;
    ...
    res_obj.label = (int)in_obstacle.meta_type % 5;
    float length_threshold = ObjectDimThreshold[res_obj.label][0];
    float width_threshold = ObjectDimThreshold[res_obj.label][1];
    float height_threshold = ObjectDimThreshold[res_obj.label][2];
    res_obj.yaw = in_obstacle.yaw;
    ...
    if ( positive_yaw < 0.087222 || positive_yaw > (M_PI/2 - 0.087222) ) {
        ...
        if ( positive_yaw < 0.087222 ) {
            ...
            if (length < length_threshold) {
                length = length_threshold;
                if (abs(max_y) > abs(min_y)) {
                    max_y = min_y + length_threshold;
                } else {
                    min_y = max_y - length_threshold;
                }
            }
            width = abs(max_x - min_x);
            if (width < width_threshold) {
                width = width_threshold;
                if (abs(max_x) > abs(min_y)) {
                    max_x = min_x + width_threshold;
                } else {
                    min_x = max_x - width_threshold;
                }
            }
        } else {
        ...
        pose_x = (max_x + min_x) * 0.5;
        pose_y = (max_y + min_y) * 0.5;
        pose_z = (max_z + min_z) * 0.5;
         std::cout << "obj: " << in_cluster.points.size() << " " << length << " " << width << " " << height << std::endl; 
    } else {
        float x1 = in_cluster.points.at(0).x;
        float y1 = in_cluster.points.at(0).y;
        ele::BoundingLine left_line(in_obstacle.yaw, x1, y1, 
                ele::BoundingLine::BoundingType::Left);
        ele::BoundingLine right_line(in_obstacle.yaw, x1, y1, 
                ele::BoundingLine::BoundingType::Right);
        ele::BoundingLine top_line(in_obstacle.yaw, x1, y1, 
                ele::BoundingLine::BoundingType::Top);
        ele::BoundingLine bottom_line(in_obstacle.yaw, x1, y1, 
                ele::BoundingLine::BoundingType::Bottom);
        min_z = in_cluster.points.at(0).z;
        max_z = in_cluster.points.at(0).z;
        for (auto pit = in_cluster.points.begin()+1; 
                pit != in_cluster.points.end(); ++pit) {
            if (pit->z > max_z)
                max_z = pit->z; 
            if (pit->z < min_z)
        ...
        pose_x = (lt_x + rb_x) * 0.5;
        pose_y = (lt_y + rb_y) * 0.5;
        pose_z = (max_z + min_z) * 0.5;
        float radial_vx = 0.0f;
        float radial_vy = 0.0f;
        float normal_vx = 0.0f;
        float normal_vy = 0.0f;
        float cos_yaw = cos(res_obj.yaw);
        float sin_yaw = sin(res_obj.yaw);        
        if (length < length_threshold) {
            float dl = (length_threshold - length) * 0.5;
            if (abs(top_line.C()) > abs(bottom_line.C())) {
                radial_vx = cos_yaw * dl;
                radial_vy = sin_yaw * dl;
            } else {
                radial_vx = -cos_yaw * dl;
                radial_vy = -sin_yaw * dl; 
            }
        ...
        if (width < width_threshold) {
            float dw = (width_threshold - width) * 0.5;
            if (abs(left_line.C()) > abs(right_line.C())) {
                normal_vx = -sin_yaw * dw;
                normal_vy = cos_yaw * dw;
            } else {
                normal_vx = sin_yaw * dw;
                normal_vy = -cos_yaw * dw; 
            }
        ...
        pose_x += normal_vx + radial_vx;
        pose_y += normal_vy + radial_vy;
    }

    res_obj.pose.x() = pose_x;
    res_obj.pose.y() = pose_y;
    res_obj.pose.z() = pose_z;
    res_obj.dims.x() = length;
    res_obj.dims.y() = width;
    res_obj.dims.z() = height;
    ...
    // LOG_DEBUG("object : %f %f %f", pose_x, pose_y, pose_z);
                           ...
                           const perception::Header & in_header)
{
         ...
         obstacle_id++)
    {
        ...
        perception::Object out_obj = obstacleToObject(*obs, in_header);
