...
#include "lidar/tracker/HungarianAlg.h"
#include <vector>
#include <string>
#include <functional>

#define ASSIGN_DEBUG_OUTPUT  0
...
/**
 * @brief 目标关联匹配类
 * @tparam LeftClass 二分图左侧目标
 * @tparam RightClass 二分图右侧目标
 */
...
public:
  void Solve() {
#if ASSIGN_DEBUG_OUTPUT
    std::cout << "calcul dist" << std::endl;
#endif // ASSIGN_DEBUG_OUTPUT
    ...
    if (block_num < 1) {
#if ASSIGN_DEBUG_OUTPUT
      std::cout << "get 0 blocks" << std::endl;
#endif // ASSIGN_DEBUG_OUTPUT
      return ;
    }
    ...
    for (size_t i=0; i<block_num; i++) {
      if (left_blocks_[i].empty() 
          || right_blocks_[i].empty() ) {
        continue;
      }

      // case : one vs one
      if (1 == left_blocks_[i].size() 
          && 1 == right_blocks_[i].size()) {
#if ASSIGN_DEBUG_OUTPUT
