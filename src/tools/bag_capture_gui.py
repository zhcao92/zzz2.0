...
parking_number_publisher = None
parking_backword_publisher = None
parking_forword_publisher = None
...
def ros_main_thread():
    ...
    try:
        ...
        global traffic_publisher, parking_number_publisher, parking_backword_publisher, parking_forword_publisher
        ...
        parking_number_publisher = rospy.Publisher('/parking_number', Int32, queue_size=10)
        parking_backword_publisher = rospy.Publisher('/parking_order', String, queue_size=10)
        parking_forword_publisher = rospy.Publisher('/parking_order', String, queue_size=10)
...
class MyViz(QWidget):
    ...
    def __init__(self):
        ...
        self.parking_number = QTextEdit()
        parking_layout.addWidget(self.parking_number)

        pullingOver_layout = QVBoxLayout()
        enter_button = QPushButton("BackWard")
        enter_button.clicked.connect(self.onBackWardButtonClick)
        pullingOver_layout.addWidget(enter_button)

        exit_button = QPushButton("ForWard")
        exit_button.clicked.connect(self.onForWardButtonClick)
        pullingOver_layout.addWidget(exit_button)

        parking_layout.addLayout(pullingOver_layout)

        leanParking_layout = QVBoxLayout()
        in_button = QPushButton("In")
        in_button.clicked.connect(self.onInButtonClick)
        leanParking_layout.addWidget(in_button)

        out_button = QPushButton("Out")
        out_button.clicked.connect(self.onOutButtonClick)
        leanParking_layout.addWidget(out_button)

        parking_layout.addLayout(leanParking_layout)
        ...
        layout.addLayout(parking_layout)
    ...
    def onBackWardButtonClick(self):
        global parking_number_publisher, parking_backword_publisher
        parking_number_publisher.publish(Int32(int(self.parking_number.toPlainText())))
        self.parking_number.clear()
        parking_backword_publisher.publish(String('backwardin'))

    def onForWardButtonClick(self):
        global parking_number_publisher, parking_forword_publisher
        #parking_number_publisher.publish(Int32(int(self.parking_number.toPlainText())))
        self.parking_number.clear()
        parking_forword_publisher.publish(String('forwardout'))

    def onInButtonClick(self):
        global parking_number_publisher, parking_backword_publisher
        parking_number_publisher.publish(Int32(int(self.parking_number.toPlainText())))
        self.parking_number.clear()
        parking_backword_publisher.publish(String('in'))

    def onOutButtonClick(self):
        global parking_number_publisher, parking_forword_publisher
        #parking_number_publisher.publish(Int32(int(self.parking_number.toPlainText())))
        self.parking_number.clear()
        parking_forword_publisher.publish(String('out'))
...
if __name__ == '__main__':
    ...
    # with open('/media/icv/odd/Record.txt', 'a+') as f:
    #     f.write('Today is :{0}\n'.format(datetime.date.today()))
    #     f.write('This testing begin from:{0}\n'.format(datetime.datetime.now().strftime('%H:%M:%S')))
    #     f.write('git commit: '+ newhexsha + '\n')
    #     f.write('---------------------------\n')
    # qt-gui
